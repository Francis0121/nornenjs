...
var CUDA_MODULE_MAP = new HashMap();
...
NornenjsServer.prototype.socketIoEvent = function(){
    ...
    this.io.sockets.on('connection', function(socket){
        ...
        socket.on('disconnect', function () {
            ...
            if(clientQueue.indexOf(socket.id) == -1){
                streamClientCount--;
                logger.debug('[Disconnect] Stream user count [' + streamClientCount + ']');
            }

            var clientId = clientQueue.shift();

            if(clientId != undefined){
                logger.debug('Connect socket client id [' + clientId + ']');
                socket.broadcast.to(clientId).emit('otherClientDisconnect');
            }
        ...
        socket.on('stream', function(){
            ...
            sqlite.db.get(sqlite.sql.volume.selectVolumeOne, { $pn: volumePn }, function(err, volume){
                ...
                if(volume == undefined) {
                    ...
                    CUDA_MODULE_MAP(clientId, 
                        setInterval(makeImageInterval, 1000/60, cudaRender, socket));
                }
            });
        });
 ...
 *  CudaRender Object
 */
function makeImageInterval(cudaRender, socket){
    ...
    socket.emit('jpeg', jpeg.encodeSync());
    cudaRender.end();
